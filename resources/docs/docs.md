...
The `home-routes` handler in the `my-blog-clj.routes.home` namespace
...
The `home-page` function will in turn call the `my-blog-clj.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `my-blog-clj.handler` namespace:
...
Request middleware functions are located under the `my-blog-clj.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `my-blog-clj.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `my-blog-clj.db.core` in some other namespace.
